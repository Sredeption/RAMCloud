#ifndef RAMCLOUD_AUXILIARYTRANSPORT_H
#define RAMCLOUD_AUXILIARYTRANSPORT_H
...
class AuxiliaryTransport : public Transport{
  PRIVATE:
    struct DataHeader;

  public:
    explicit AuxiliaryTransport(Context* context, const ServiceLocator* locator,
  ...
  PRIVATE:
    ...
    class Session : public Transport::Session {
      ...
      PRIVATE:
        ...
        Session(AuxiliaryTransport * t, const ServiceLocator* locator,
                uint32_t timeoutMs);

        friend class AuxiliaryTransport;
    ...
    class MessageAccumulator {
      public:
        MessageAccumulator(AuxiliaryTransport * t, Buffer* buffer);
        ~MessageAccumulator();
        bool addPacket(DataHeader *header, uint32_t length);
        uint32_t requestRetransmission(AuxiliaryTransport *t,
    ...
    class OutgoingMessage {
      public:
        ...
        OutgoingMessage(ClientRpc* clientRpc, ServerRpc* serverRpc,
                AuxiliaryTransport* t, Buffer* buffer,
                const Driver::Address* recipient)
            : buffer(buffer)
            , clientRpc(clientRpc)
            , serverRpc(serverRpc)
    ...
    class Poller : public Dispatch::Poller {
      public:
        explicit Poller(Context* context, AuxiliaryTransport* t)
            : Dispatch::Poller(context->auxDispatch, "AuxiliaryTransport(" +
  ...
  PRIVATE:
    ...
    DISALLOW_COPY_AND_ASSIGN(AuxiliaryTransport);
};

}

#endif //RAMCLOUD_AUXILIARYTRANSPORT_H
