...
struct MigrationSegmentBuilderTest : public ::testing::Test {
    Context context;
    ...
    MigrationSegmentBuilderTest()
        : context(), partitions(), serverId(99, 0), serverList(&context),
          serverConfig(ServerConfig::forTesting()),
          replicaManager(&context, &serverId, 0, false, false),
          masterTableMetadata(), allocator(&serverConfig),
    ...
    DISALLOW_COPY_AND_ASSIGN(MigrationSegmentBuilderTest);
};


TEST_F(MigrationSegmentBuilderTest, build)
{
    auto build = MigrationSegmentBuilder::build;
...
TEST_F(MigrationSegmentBuilderTest, build_safeVersionEntries)
{
    auto build = MigrationSegmentBuilder::build;
...
TEST_F(MigrationSegmentBuilderTest, build_participantList)
{
    auto build = MigrationSegmentBuilder::build;
...
TEST_F(MigrationSegmentBuilderTest, extractDigest)
{
    auto extractDigest = MigrationSegmentBuilder::extractDigest;
    LogSegment *segment = segmentManager.allocHeadSegment();
    SegmentCertificate certificate;
    uint32_t length = segment->getAppendedLength(&certificate);
...
TEST_F(MigrationSegmentBuilderTest, isEntryAlive)
{
    auto isEntryAlive = MigrationSegmentBuilder::isEntryAlive;
...
TEST_F(MigrationSegmentBuilderTest, whichPartition)
{
    auto whichPartition = MigrationSegmentBuilder::whichPartition;
