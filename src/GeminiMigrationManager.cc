...
GeminiMigrationManager::~GeminiMigrationManager()
{
    for (auto migration = migrationsInProgress.begin();
        ...
        GeminiMigration *currentMigration = *migration;

        // If this migration has not completed, wait for it to do so.
        while (currentMigration->phase != GeminiMigration::COMPLETED) {
...
uint64_t GeminiMigrationManager::updateRegularPullProgress(uint32_t i)
{
    for (auto &migration : migrationsInProgress) {
...
GeminiMigration::GeminiMigration(Context *context,
    ...
    : context(context), tabletManager(), objectManager(),
      ...
      sideLogCommitRpc(), migrationStartTS(), migrationEndTS(),
      migratedMegaBytes(), sideLogCommitStartTS(), sideLogCommitEndTS()
{
...
GeminiMigration::~GeminiMigration()
{
...
bool
GeminiMigration::addPriorityHash(uint64_t priorityHash)
{
    ...
    if (takeOwnershipRpc) {
        ...
        if (getHeadOfLogRpc->isReady()) {
            ...
            RAMCLOUD_LOG(ll, "Added tablet[0x%lx, 0x%lx] in table %lu with"
                             ...
                             " creation time is [%lu, %u]", startKeyHash,
                         endKeyHash,
            ...
            takeOwnershipRpc.construct(context, tableId, startKeyHash,
                                       endKeyHash,
                                       (context->getMasterService())->serverId,
...
int
GeminiMigration::pullAndReplay_main()
{
    int workDone = 0;
...
GeminiMigration::pullAndReplay_priorityHashes()
{
    SpinLock::Guard lock(priorityLock);
    ...
    if (priorityPullRpc) {
        if (priorityPullRpc->isReady()) {
            ...
            priorityHashesResponseBuffer->truncateFront(sizeof32(
                ...
                LOG(ll,
                    "Priority hashes request returned %u log entries. Issuing"
            ...
            context->workerManager->handoffRpc(priorityReplayRpc.get());
...
__inline __attribute__((always_inline)) int
GeminiMigration::pullAndReplay_reapPullRpcs()
{
    int workDone = 0;
...
__inline __attribute__((always_inline)) int
GeminiMigration::pullAndReplay_reapReplayRpcs()
{
    int workDone = 0;
...
__inline __attribute__((always_inline)) int
GeminiMigration::pullAndReplay_sendPullRpcs()
{
    int workDone = 0;
    ...
    if (freePullRpcs.size() != 0) {
        ...
        while ((freePullRpcs.size() != 0) &&
            ...
            (*pullRpc).construct(context, sourceSession, tableId, startKeyHash,
                                 ...
                                 endHTBucket,
                                 20 * 1024 /* Ask the source for 20 KB */,
...
__inline __attribute__((always_inline)) int
GeminiMigration::pullAndReplay_sendReplayRpcs()
{
    int workDone = 0;
    ...
    if (freeReplayRpcs.size() != 0) {
        ...
        while ((freeReplayRpcs.size() != 0) &&
            ...
            (*replayRpc).construct(partition, responseBuffer, sideLog,
                                   localLocator, certificate);
            context->workerManager->handoffRpc(replayRpc->get());
    ...
    if (!sideLogCommitRpc) {
        ...
        if (nextSideLogCommit < MAX_PARALLEL_REPLAY_RPCS) {
            sideLogCommitRpc.construct(&(sideLogs[nextSideLogCommit]),
                                       localLocator);
            context->workerManager->handoffRpc(sideLogCommitRpc.get());
